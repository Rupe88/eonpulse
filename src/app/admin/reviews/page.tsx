...
export default function AdminReviewsPage() {
  ...
  useEffect(() => {
    ...
    if (status !== "authenticated" || !user) {
      replaceOrHardNavigate(router, "/login?next=%2Fadmin%2Freviews", "/admin/reviews");
      return;
    }
  ...
  if (!ready) return <AppShellSkeleton headerTitle="Reviews" headerSubtitle="Loading reviews module" />;

  if (status !== "authenticated" || !user) {
    return (
      <div className="flex min-h-full flex-1 flex-col items-center justify-center gap-4 bg-[var(--color-canvas)] px-4">
        <Spinner />
        <Link href="/login?next=%2Fadmin%2Freviews" className="rounded-md bg-neutral-900 px-4 py-2 text-sm font-medium text-white hover:bg-neutral-800">
          Sign in
        </Link>
      </div>
    );
  ...
  return (
    <AppShell headerTitle="Reviews" headerSubtitle="Review queues, threads, and approvals" headerClassName="px-6" mainClassName="flex-1 px-6 py-8">
      <ReviewsPanel />
    </AppShell>
  );
}
