...
import { ProgressPanel } from "@/components/admin/progress-panel";
...
export default function AdminProgressPage() {
  ...
  useEffect(() => {
    ...
    if (status !== "authenticated" || !user) {
      replaceOrHardNavigate(router, "/login?next=%2Fadmin%2Fprogress", "/admin/progress");
      return;
    }
    if (!canAccessAdminPanel(user.role)) router.replace("/dashboard");
  }, [ready, status, user, router]);

  if (!ready) return <AppShellSkeleton headerTitle="Project Progress" headerSubtitle="Loading analytics module" />;

  if (status !== "authenticated" || !user) {
    return (
      <div className="flex min-h-full flex-1 flex-col items-center justify-center gap-4 bg-[var(--color-canvas)] px-4">
        <Spinner />
        <Link href="/login?next=%2Fadmin%2Fprogress" className="rounded-md bg-neutral-900 px-4 py-2 text-sm font-medium text-white hover:bg-neutral-800">
          Sign in
        </Link>
      </div>
    );
  ...
  return (
    <AppShell
      ...
      headerSubtitle="KPIs, milestone velocity, overdue and blocker analytics"
